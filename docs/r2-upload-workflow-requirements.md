...
这份文档定义 CF Studio 下一阶段的 R2 上传能力：上传配置档、批量上传、上传前图片处理，以及上传后链接复制格式。

产品方向不是复制 PicGo 或 PicList 的完整功能，而是围绕 Cloudflare R2 做一个更好用的本地素材上传和管理工具。核心场景是截图、博客图片、文档图片、静态资源和私有/公开 Bucket 的日常维护。
...
- 对 CF Studio 来说，自定义域名应该优先于 `r2.dev`，`r2.dev` 不应被当作生产默认方案。
