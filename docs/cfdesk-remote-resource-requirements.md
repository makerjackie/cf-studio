...
这份文档用于明确 CFDesk 接下来的产品主线、功能优先级和验收标准。产品名已经从 CF Studio 切换为 CFDesk。

CFDesk 的核心方向是远程 Cloudflare 资源管理。本地开发能力不重复建设，统一通过 Cloudflare 官方 Local Explorer 做入口和指引。

## 背景判断

FlareDesk 的价值在于围绕 `wrangler dev` 提供本地开发控制台，包括项目识别、绑定侧边栏、请求调试、绑定调试、快照和 Tunnel 等能力。这条路线适合本地 Workers 开发。

CFDesk 不应该复刻 FlareDesk。Cloudflare 官方已经提供 Local Explorer，用于在本地开发时浏览和编辑本地绑定数据。CFDesk 更适合把精力放在远程账号资源上，解决用户平时反复打开 Cloudflare Dashboard 才能完成的高频工作。
...
CFDesk 是面向 Cloudflare 日常工作的桌面工具。它帮助用户在一个低干扰界面中完成远程资源查看、上传、调试、导出、日志检查和确认式安全操作。
...
目标：把 CFDesk 的主线固定为远程 Cloudflare 资源管理，并保留 Local Explorer 官方入口。
...
目标：让 R2 成为 CFDesk 最明显的高频价值点，适合用作图床、博客素材库、静态资源管理器和对象检查工具。
...
目标：让 Workers 成为 CFDesk 的第二个高频主模块，先从低风险只读运维能力开始。
...
- 侧边栏和文档都能清楚表达：CFDesk 走远程资源管理路线。
- Local Explorer 只作为官方本地工具入口，不变成 CFDesk 自研本地控制台。
